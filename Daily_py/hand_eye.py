...
def get_RT_from_chessboard(img_path, chess_board_x_num, chess_board_y_num, K, chess_board_len):
    ...
    size = gray.shape[::-1]
    ret, corners = cv2.findChessboardCorners(gray, (chess_board_x_num - 1, chess_board_y_num - 1), None)
    # print(corners)
...
folder = r"D:\WeChat\WeChat Files\wxid_7x9ow40uxl9x21\FileStorage\File\2023-06\realsense2UR_calibData\realsense2UR_calibData"  # 棋盘格图片存放文件夹
...
for i in good_picture:
    # print(i)
    image_path = folder + '\\' + str(4) + '_Color.png'
...
for i in good_picture:
    ...
    RT = np.linalg.inv(pose_robot(sheet_1.iloc[i - 1]['ax'], sheet_1.iloc[i - 1]['ay'], sheet_1.iloc[i - 1]['az'],
                                  sheet_1.iloc[i - 1]['dx'],
                                  sheet_1.iloc[i - 1]['dy'], sheet_1.iloc[i - 1]['dz']))
