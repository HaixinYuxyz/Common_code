...
class WindowCrossAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        qkv_1 = self.qkv_branch_1(branch_1).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1,
                                                                                                           4)
        qkv_2 = self.qkv_branch_2(branch_2).reshape(B_, N, 3, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1,
                                                                                                           4)
...
class SwinCrossAttentionBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        x_windows_branch_1 = x_windows_branch_1.view(-1, self.window_size * self.window_size,
                                                     C)  # nW*B, window_size*window_size, C
        x_windows_branch_2 = x_windows_branch_2.view(-1, self.window_size * self.window_size,
                                                     C)  # nW*B, window_size*window_size, C
...
if __name__ == '__main__':
    cross_attention = CrossAttention(in_channel=96, depth=1, num_heads=1)
    rgb_feature = torch.randn((1, 96, 56, 56))
    depth_feature = torch.randn((1, 96, 56, 56))
    out = cross_attention(tuple([rgb_feature, depth_feature]))
    print(out)
